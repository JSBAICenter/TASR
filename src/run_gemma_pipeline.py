"""End-to-end Gemma cross-model replication pipeline on HotpotQA.

Mirror of src/run_devstral_pipeline.py with Gemma env + namespace
(results/gemma/, figures/gemma/). Adds a 3rd model family to the
cross-model transfer story.

Run in background so it survives a disconnect:
    nohup venv/bin/python src/run_gemma_pipeline.py \\
        > logs/gemma_pipeline.log 2>&1 &
    disown

Resumable: every stage checks whether its output artifact exists and
skips if so.
"""
...
GEMMA_ENV = {
    "LLM_BASE_URL": os.getenv("GEMMA_BASE_URL", "https://your-gemma-endpoint.example/v1"),
    "LLM_MODEL": os.getenv("GEMMA_MODEL", "google/gemma-4-31B-it"),
    "LLM_API_KEY": os.getenv("GEMMA_API_KEY", "REPLACE_ME"),
    "LLM_NO_THINKING_KWARG": "true",
}
for k, v in GEMMA_ENV.items():
    os.environ[k] = v
...
NS = Path("results/gemma")
FIG_NS = Path("figures/gemma")
...
def stage_instrumented_logprobs() -> None:
    ...
    for data, out in pairs:
        if out.exists():
            ...
            continue
        log(f"  starting {data} -> {out} (this is the heavy stage)")
...
def stage_offline_eval() -> None:
    ...
    for k, pq_b in fixed.items():
        ...
        diff_f1 = np.zeros(n_boot); diff_em = np.zeros(n_boot); diff_calls = np.zeros(n_boot)
        ours_f1_b = np.zeros(n_boot); base_f1_b = np.zeros(n_boot)
...
def stage_offline_ablations() -> None:
    log("STAGE 8: 876-DNF ablation on lp parquet")
...
def stage_figures_and_qualitative() -> None:
    log("STAGE 10: Pareto figure on Gemma lp parquet")
    ...
    log("STAGE 11: qualitative dilution analysis on Gemma lp parquet")
...
def main() -> None:
    log("=" * 60)
    log("Gemma cross-model replication pipeline (HotpotQA)")
