"""Run the Devstral instrumented-lp trace on 2WikiMultiHopQA (cross-dataset transfer).

Reuses run_instrumented_lp from src/run_instrumented_with_logprobs.py without any
changes. Outputs are namespaced under results/devstral_2wiki/. After the trace
finishes, computes the headline `answer_stable == 1` rule with paired-bootstrap
CIs vs fixed-k baselines, plus a per-question-type breakdown.

Run in background so it survives a disconnect:
    nohup python src/run_devstral_2wiki.py > logs/devstral_2wiki.log 2>&1 &
    disown
"""
...
# Force Devstral env BEFORE importing llm_client.
DEVSTRAL_ENV = {
...
def main() -> None:
    log("=" * 60)
    log("Devstral on 2WikiMultiHopQA (cross-dataset transfer)")
    ...
    log("=" * 60)
    log("Headline on 2Wiki dev_eval (Devstral)")
    log("=" * 60)
