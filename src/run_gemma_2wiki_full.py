"""End-to-end Gemma pipeline on 2WikiMultiHopQA (cross-dataset, full ablation).

Mirror of src/run_devstral_2wiki_full.py with Gemma env + namespace
(results/gemma_2wiki/, figures/gemma_2wiki/).

Run in background so it survives a disconnect:
    nohup venv/bin/python src/run_gemma_2wiki_full.py \\
        > logs/gemma_2wiki_full.log 2>&1 &
    disown
...
NS = Path("results/gemma_2wiki")
FIG_NS = Path("figures/gemma_2wiki")
...
def stage_offline_eval() -> None:
    ...
    if out_bootstrap.exists():
        log(f"  [skip] {out_bootstrap} exists (preserves any earlier per-type breakdown)")
        return
...
def stage_figures_and_qualitative() -> None:
    log("STAGE 10: Pareto figure on 2Wiki + Gemma lp parquet")
    ...
    log("STAGE 11: qualitative dilution analysis on 2Wiki + Gemma lp parquet")
...
def main() -> None:
    log("=" * 60)
    log("Gemma on 2WikiMultiHopQA (full pipeline)")
